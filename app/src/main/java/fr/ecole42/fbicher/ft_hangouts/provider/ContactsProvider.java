package fr.ecole42.fbicher.ft_hangouts.provider;
...
/**
 * Provides access to a database of contactss. Each contact has a last name,
 * a creation date and a modified data.
 * <p>
 * Created by fbicher on 1/24/16.
 */
public class ContactsProvider extends ContentProvider {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            */
            case ENTRY_ID:
                qb.setProjectionMap(sContactsProjectionMap);
                qb.appendWhere(
                        ContactsContract.Contacts._ID +    // the name of the ID column
                                "=" +
    ...
    public String getType(@NonNull Uri uri) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            // If the pattern is for entries, returns the general content type.
            case ENTRIES:
    ...
    public Uri insert(@NonNull Uri uri, ContentValues initialValues) {
        ...
        // If the values map doesn't contain an avatar, sets the value to an empty string.
        ...
        // If the values map doesn't contain a name, sets the value to an empty string.
        if (!values.containsKey(ContactsContract.Contacts.COLUMN_NAME_NAME)) {
            values.put(ContactsContract.Contacts.COLUMN_NAME_NAME, "");
        }

        // If the values map doesn't contain a last name, sets the value to an empty string.
        ...
        long rowId = db.insert(
                ContactsContract.Contacts.TABLE_NAME,               // The table to insert into.
                ContactsContract.Contacts.COLUMN_NAME_NAME,         // A hack, SQLite sets this
                // column value to null if values is empty.
        ...
        if (rowId > 0) {
            ...
            Uri taskUri = ContentUris.withAppendedId(ContactsContract.Contacts.CONTENT_ID_URI_BASE,
                    rowId);
     ...
     * this method deletes the one record specified by the ID in the URI. Otherwise, it deletes
    ...
    public int delete(@NonNull Uri uri, String where, String[] whereArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case ENTRIES:
                count = db.delete(
                        ContactsContract.Contacts.TABLE_NAME,  // The database table name
            ...
            case ENTRY_ID:
                 ...
                 */
                finalWhere =
                        ContactsContract.Contacts._ID +                 // The ID column name
                                " = " +                                 // test for equality
     ...
     * @param where     An SQL "WHERE" clause that selects records based on their column values.
     *                  If this is null, then all records that match the URI pattern are selected.
    ...
    public int update(@NonNull Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case ENTRY_ID:
                ...
                // Starts creating the final WHERE clause by restricting it to the incoming entry ID.
                finalWhere =
                        ContactsContract.Contacts._ID +     // The ID column name
                                " = " +                     // test for equality
                                uri.getPathSegments().      // the incoming entry ID
